...
describe('PaginatedDataSource', () => {
  it('loads initial page on construction (default triggerInitialFetch)', async () => {
    ...
    const items = await firstValueFrom(ds.connect().pipe(filter(arr => arr.length > 0), take(1)));
  ...
  it('concatenates items when concatData=true (default) on next page', async () => {
    ...
    await firstValueFrom(ds.connect().pipe(filter(arr => arr.length > 0), take(1)));
    ds.loadNextPage();
    const items2 = await firstValueFrom(ds.connect().pipe(filter(arr => arr.length === 4), take(1)));
  ...
  it('replaces items when concatData=false', async () => {
    ...
    const items1 = await firstValueFrom(ds.connect().pipe(filter(arr => arr.length > 0), take(1)));
    expect(items1).toEqual([1, 2]);
    ds.loadNextPage();
    const items2 = await firstValueFrom(ds.connect().pipe(filter(arr => arr.length === 2 && arr[0] === 3), take(1)));
  ...
  it('resets pagination and loads first page on setQuery', async () => {
    ...
    await firstValueFrom(ds.connect().pipe(filter(arr => arr.length > 0), take(1)));
    ds.loadNextPage();
    await firstValueFrom(ds.connect().pipe(filter(arr => arr.length === 4), take(1)));

    ds.setQuery('abc');
    const itemsAfterQuery = await firstValueFrom(ds.connect().pipe(filter(arr => arr.length === 2), take(1)));
